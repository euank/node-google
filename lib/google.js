...
var igoogle = function(query, start, callback) {
  ...
  request(rOpt, function(err, resp, body) {
    if ((err === null) && resp.statusCode === 200) {
      ...
      var resultStatsElem = $(resultStatsSel);
      var resultStats = 0;
      try {
        resultStats = parseInt(resultStatsElem.text().match(/.*?([\d,]+) results?/)[1].replace(/,/g,''));
      } catch(ex){}
      ...
      callback(null, nextFunc, links, resultStats);
    } else {
      e = new Error();
